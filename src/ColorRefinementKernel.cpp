...
namespace ColorRefinement {
    ...
    GramMatrix ColorRefinementKernel::compute_gram_matrix(const uint num_iterations, bool use_labels) {
        ...
        for (Graph &graph: m_graph_database) {
            color_counters.push_back(move(compute_colors(graph, num_iterations, use_labels)));
        }
